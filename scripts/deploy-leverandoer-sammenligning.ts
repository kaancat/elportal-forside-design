...
async function createLeverandoerSammenligning() {
  console.log('🚀 Starting deployment of Leverandør Sammenligning page...')

  const pageContent = {
    _id: 'page.leverandoer-sammenligning',
    _type: 'page',
  ...
  try {
    console.log('📝 Creating/updating page document...')
    ...
    console.log('\n📊 Page Statistics:')
    console.log(`- Components used: ${pageContent.contentBlocks.length}`)
    console.log(`- SEO keywords: ${pageContent.seoKeywords.length}`)
    console.log(`- Word count: ~3200+`)
    console.log('- Special components: ProviderList, DeclarationGridmix, ConsumptionMap, PriceCalculator')
...
// Execute deployment
createLeverandoerSammenligning()
  .then(() => {
