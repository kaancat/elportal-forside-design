...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  const { region = 'DK1', date } = req.query;
  ...
  const API_URL = `https://api.energidataservice.dk/dataset/Forecasts_Hour?start=${start}&end=${end}&filter={"PriceArea":["${region}"]}&sort=HourUTC asc`;
