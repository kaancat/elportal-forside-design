...
const RenewableEnergyForecast: React.FC<RenewableEnergyForecastProps> = ({ block }) => {
  ...
  // Back to a single, clear region selection
  const [selectedRegion, setSelectedRegion] = useState<'Danmark' | 'DK1' | 'DK2'>('Danmark');

  useEffect(() => {
    // API Route now correctly fetches all data, and we filter on the frontend
  ...
  const processedData = useMemo(() => {
    const dataForRegion = selectedRegion === 'Danmark' ? allData : allData.filter(d => d.PriceArea === selectedRegion);

    const grouped = dataForRegion.reduce((acc, record) => {
        const hourKey = new Date(record.HourUTC).toLocaleTimeString('da-DK', { hour: '2-digit', minute: '2-digit' }).replace(/\./g, ':');
        if (!acc[hourKey]) {
            acc[hourKey] = { 'Solar': 0, 'Onshore Wind': 0, 'Offshore Wind': 0 };
        }
        if (record.ForecastDayAhead !== null) {
          acc[hourKey][record.ForecastType] += record.ForecastDayAhead;
        }
        return acc;
    }, {} as Record<string, { 'Solar': number; 'Onshore Wind': number; 'Offshore Wind': number; }>);
  ...
  const yAxisMax = useMemo(() => {
    ...
    if (dataMax === 0) return 1000; // Prevent flat line if all data is 0
    const magnitude = Math.pow(10, Math.floor(Math.log10(dataMax)));
    return Math.ceil(dataMax / (magnitude / 2)) * (magnitude / 2); // Round to nearest half-magnitude
  }, [processedData]);
